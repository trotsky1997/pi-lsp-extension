...
import { getOrCreateAstService } from "./tree-sitter-wasm-core.js";
...
function buildSpawnEnv(settings: LSPServerSettings): NodeJS.ProcessEnv {
  return settings.env ? { ...process.env, ...settings.env } : process.env;
}

function resolveRootWithOverride(
  file: string,
  cwd: string,
  ...
  if (settings.rootMarkers && settings.rootMarkers.length > 0) {
    return findRoot(file, cwd, settings.rootMarkers);
  }
  return fallback();
}

function withServerSettings(
  settings: LSPServerSettings,
  result: { process: ChildProcessWithoutNullStreams; initializationOptions?: Record<string, unknown>; workspaceConfiguration?: Record<string, unknown> } | undefined,
) {
  if (!result) return undefined;
  return {
    ...result,
    initializationOptions: settings.initializationOptions ?? result.initializationOptions,
    workspaceConfiguration: settings.workspaceConfiguration ?? result.workspaceConfiguration,
  };
}

async function spawnChecked(cmd: string, args: string[], cwd: string, settings: LSPServerSettings): Promise<ChildProcessWithoutNullStreams | undefined> {
  try {
    const child = spawn(cmd, args, { cwd, env: buildSpawnEnv(settings), stdio: ["pipe", "pipe", "pipe"] });
...
async function spawnWithFallback(cmd: string, argsVariants: string[][], cwd: string, settings: LSPServerSettings): Promise<ChildProcessWithoutNullStreams | undefined> {
  for (const args of argsVariants) {
    const child = await spawnChecked(cmd, args, cwd, settings);
    if (child) return child;
  }
  return undefined;
}

async function spawnConfiguredCommand(
  settings: LSPServerSettings,
  root: string,
  fallback: () => Promise<{ process: ChildProcessWithoutNullStreams; initializationOptions?: Record<string, unknown>; workspaceConfiguration?: Record<string, unknown> } | undefined>,
  defaultArgs: string[] = ["--stdio"],
): Promise<{ process: ChildProcessWithoutNullStreams; initializationOptions?: Record<string, unknown>; workspaceConfiguration?: Record<string, unknown> } | undefined> {
  if (settings.disabled) return undefined;
  if (settings.command) {
    const child = await spawnChecked(settings.command, settings.args ?? defaultArgs, root, settings);
    return withServerSettings(settings, child ? { process: child } : undefined);
  }
  return withServerSettings(settings, await fallback());
}
...
async function spawnKotlinLanguageServer(root: string, settings: LSPServerSettings): Promise<ChildProcessWithoutNullStreams | undefined> {
  ...
  return spawnWithFallback(kls, [[]], root, settings);
}

async function spawnSourcekitLsp(root: string, settings: LSPServerSettings): Promise<ChildProcessWithoutNullStreams | undefined> {
  const direct = which("sourcekit-lsp");
  if (direct) return spawnWithFallback(direct, [[], ["--stdio"]], root, settings);
...
function vscodeExtensionCandidates(rootDir: string, prefix: string): string[] {
  try {
    return fs.readdirSync(rootDir, { withFileTypes: true })
      .filter((entry: fs.Dirent) => entry.isDirectory() && entry.name.startsWith(prefix))
      .map((entry: fs.Dirent) => entry.name)
      .sort()
      .reverse()
      .map((name: string) => path.join(rootDir, name));
  } catch {
    return [];
  }
}
...
export const LSP_SERVERS: LSPServerConfig[] = [
  ...
  { id: "texlab", extensions: [".tex", ".bib", ".sty", ".cls"], findRoot: markerRoot(["texlabroot", "Tectonic.toml", ".latexmkrc", ".git"]), spawn: simpleSpawn("texlab") },
  {
    ...
    spawn: async (root, settings) => spawnConfiguredCommand(settings, root, async () => {
      let dart = which("dart");
  ...
  { id: "ruby-lsp", extensions: [".rb", ".erb"], findRoot: markerRoot(["Gemfile", ".ruby-version"]), spawn: simpleSpawn("ruby-lsp") },
  {
    ...
    spawn: async (root, settings) => spawnConfiguredCommand(settings, root, async () => {
      ...
      if (!cmd) return undefined;
      return { process: spawn(cmd, ["--stdio"], { cwd: root, env: buildSpawnEnv(settings), stdio: ["pipe", "pipe", "pipe"] }) };
    }),
  },
  ...
  { id: "gopls", extensions: [".go"], findRoot: (f, cwd, settings) => resolveRootWithOverride(f, cwd, settings, () => findRoot(f, cwd, ["go.work"]) || findRoot(f, cwd, ["go.mod"])), spawn: simpleSpawn("gopls", []) },
  {
    ...
    findRoot: (f, cwd, settings) => resolveRootWithOverride(f, cwd, settings, () => findRootKotlin(f, cwd)),
    spawn: async (root, settings) => spawnConfiguredCommand(settings, root, async () => {
      const proc = await spawnKotlinLanguageServer(root, settings);
  ...
  },
  {
    ...
    findRoot: (f, cwd, settings) => resolveRootWithOverride(f, cwd, settings, () => findRootSwift(f, cwd)),
    spawn: async (root, settings) => spawnConfiguredCommand(settings, root, async () => {
      const proc = await spawnSourcekitLsp(root, settings);
      if (!proc) return undefined;
      return { process: proc };
    }),
  },
...
export class LSPManager {
  ...
  private async initClient(config: LSPServerConfig, root: string): Promise<LSPClient | undefined> {
    ...
    try {
      const settings = loadResolvedLspSettings(this.cwd).servers[config.id] ?? {};
      const handle = await config.spawn(root, settings);
      ...
      const client: LSPClient = {
        ...
        stderr,
        root,
        workspaceConfiguration: handle.workspaceConfiguration,
        closed: false,
      };
      ...
      conn.onRequest("workspace/configuration", (params: { items?: Array<{ section?: string }> }) => {
        const configuration = handle.workspaceConfiguration ?? {};
        const items = params.items ?? [];
        return items.map((item) => {
          if (!item.section) return configuration;
          return item.section.split(".").reduce<unknown>((current, part) => {
      ...
      if (handle.workspaceConfiguration) {
        conn.sendNotification("workspace/didChangeConfiguration", { settings: handle.workspaceConfiguration });
      }
      return client;
  ...
  private isPathWithinRoot(filePath: string, root: string): boolean {
    const relativePath = path.relative(root, filePath);
    return relativePath === "" || (!relativePath.startsWith("..") && !path.isAbsolute(relativePath));
  }

  private async stopClient(client: LSPClient): Promise<void> {
    ...
    if (!wasClosed) {
      ...
      try { void client.connection.sendNotification("exit").catch(() => {}); } catch {}
    }

    try { client.connection.end(); } catch {}
    try { client.process.kill(); } catch {}
  }

  private getClientsForWorkspacePath(filePath: string): Array<[string, LSPClient]> {
    const absPath = this.resolve(filePath);
    return Array.from(this.clients.entries()).filter(([, client]) => this.isPathWithinRoot(absPath, client.root));
  }

  private getTextSyncKind(client: LSPClient): number {
  ...
  private supportsOperationForClient(client: LSPClient, operation: string): boolean {
    ...
    switch (operation) {
      ...
      case "documentHighlight": return Boolean(capabilities.documentHighlightProvider);
      case "workspaceSymbol": return Boolean(capabilities.workspaceSymbolProvider);
      case "goToImplementation": return Boolean(capabilities.implementationProvider);
      case "typeDefinition": return Boolean(capabilities.typeDefinitionProvider);
      case "prepareCallHierarchy":
      case "incomingCalls":
      case "outgoingCalls":
        return Boolean(capabilities.callHierarchyProvider);
      case "signatureHelp": return Boolean(capabilities.signatureHelpProvider);
      case "rename": return Boolean(capabilities.renameProvider);
      case "prepareRename": {
        if (!capabilities.renameProvider) return false;
        if (typeof capabilities.renameProvider === "boolean") return false;
        return Boolean(capabilities.renameProvider.prepareProvider);
      }
      case "codeAction": return Boolean(capabilities.codeActionProvider);
      case "foldingRange": return Boolean(capabilities.foldingRangeProvider);
      default: return true;
    }
  }
  ...
  async getOperationBackend(filePath: string, operation: string): Promise<OperationBackend | null> {
    ...
    if (operation === "diagnostics") {
      ...
      return getOrCreateAstService().supportsOperation(absPath, "diagnostics") ? "tree-sitter" : null;
    }
    if (clients.some(client => !client.closed && this.supportsOperationForClient(client, operation))) {
      return "lsp";
    }
    return getOrCreateAstService().supportsOperation(absPath, operation) ? "tree-sitter" : null;
  }

  async notifyWorkspaceFileEvent(filePath: string, type: FileChangeType = FileChangeType.Changed): Promise<number> {
    const absPath = this.resolve(filePath);
    const uri = pathToFileURL(absPath).href;
    const clients = this.getClientsForWorkspacePath(absPath).map(([, client]) => client);
  ...
  async restartClientsForPath(filePath: string): Promise<number> {
    const matches = this.getClientsForWorkspacePath(filePath);
  ...
  private getSupportedClients(clients: LSPClient[], operation: string): LSPClient[] {
    return clients.filter(client => !client.closed && this.supportsOperationForClient(client, operation));
  }

  private positionAt(text: string, offset: number): { line: number; character: number } {
    let line = 0;
    let character = 0;
  ...
  private buildIncrementalChange(previous: string, next: string): { range: { start: { line: number; character: number }; end: { line: number; character: number } }; text: string } | null {
    ...
    return {
      range: {
        start: this.positionAt(previous, start),
        end: this.positionAt(previous, previousEnd),
      },
  ...
  private async openOrUpdate(clients: LSPClient[], absPath: string, uri: string, langId: string, content: string, evict = true) {
    ...
    for (const client of clients) {
      ...
      try {
        if (state) {
          const change = this.getTextSyncKind(client) === 2
            ? this.buildIncrementalChange(state.content, content)
            : null;

          if (change || state.content !== content) {
            const v = state.version + 1;
            client.openFiles.set(absPath, { version: v, lastAccess: now, content });
            void client.connection.sendNotification(DidChangeTextDocumentNotification.type, {
              textDocument: { uri, version: v },
              contentChanges: change ? [change] : [{ text: content }],
            }).catch(() => {});
          } else {
            client.openFiles.set(absPath, { ...state, lastAccess: now });
          }
        } else {
          ...
          client.openFiles.set(absPath, { version: 1, lastAccess: now, content });
  ...
  isFileOpen(filePath: string): boolean {
    const absPath = this.resolve(filePath);
    for (const client of this.clients.values()) {
      if (client.openFiles.has(absPath)) return true;
    }
    return false;
  }
  ...
  async openFile(filePath: string, content?: string): Promise<boolean> {
    ...
    const clients = await this.getClientsForFile(absPath);
    if (!clients.length) {
      return getOrCreateAstService().supportsOperation(absPath, "diagnostics");
    }
  ...
  async sendRequest<T>(
    ...
    for (const client of clients) {
      if (client.closed) continue;
      try {
        const result = await client.connection.sendRequest(method, params);
        sawSuccess = true;
        if (strategy === "first") {
          return result as T;
        }
    ...
    if (!sawSuccess) return undefined;
  ...
  async touchFileAndWait(filePath: string, timeoutMs: number): Promise<{ diagnostics: Diagnostic[]; receivedResponse: boolean; unsupported?: boolean; error?: string }> {
    ...
    const clients = await this.getClientsForFile(absPath);
    if (!clients.length) {
      const treeSitter = getOrCreateAstService();
  ...
  async getDefinition(fp: string, line: number, col: number): Promise<Location[]> {
    ...
    if (!l) return getOrCreateAstService().getDefinition(this.resolve(fp), line, col);
    ...
    if (!supportedClients.length) return getOrCreateAstService().getDefinition(l.absPath, line, col);
    ...
    if (this.shouldUseMarkdownFallback(l.absPath) && (flattened.length === 0 || flattened.every((location) => !this.locationHasMeaningfulRange(location)))) {
      return getOrCreateAstService().getDefinition(l.absPath, line, col);
    }
    return flattened;
  }
  ...
  async getReferences(fp: string, line: number, col: number): Promise<Location[]> {
    const l = await this.loadFile(fp);
    if (!l) return getOrCreateAstService().getReferences(this.resolve(fp), line, col);
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    const supportedClients = this.getSupportedClients(l.clients, "findReferences");
    if (!supportedClients.length) return getOrCreateAstService().getReferences(l.absPath, line, col);
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
    ...
    if (this.shouldUseMarkdownFallback(l.absPath) && flattened.length === 0) {
      return getOrCreateAstService().getReferences(l.absPath, line, col);
    }
    return flattened;
  }
  ...
  async getHover(fp: string, line: number, col: number): Promise<Hover | null> {
    ...
    if (!l) {
      ...
      const treeSitterHover = getOrCreateAstService().getHover(absPath, line, col);
    ...
    const treeSitterHover = getOrCreateAstService().getHover(l.absPath, line, col);
  ...
  async getSignatureHelp(fp: string, line: number, col: number): Promise<SignatureHelp | null> {
    ...
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    for (const c of this.getSupportedClients(l.clients, "signatureHelp")) {
      if (c.closed) continue;
  ...
  async getDocumentSymbols(fp: string): Promise<DocumentSymbol[]> {
    const l = await this.loadFile(fp);
    if (!l) return getOrCreateAstService().getDocumentSymbols(this.resolve(fp));
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const supportedClients = this.getSupportedClients(l.clients, "documentSymbol");
    if (!supportedClients.length) return getOrCreateAstService().getDocumentSymbols(l.absPath);
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
  ...
  async getDocumentHighlights(fp: string, line: number, col: number): Promise<DocumentHighlight[]> {
    const l = await this.loadFile(fp);
    if (!l) return getOrCreateAstService().getDocumentHighlights(this.resolve(fp), line, col);
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    const supportedClients = this.getSupportedClients(l.clients, "documentHighlight");
    if (!supportedClients.length) return getOrCreateAstService().getDocumentHighlights(l.absPath, line, col);
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
        const response = await c.connection.sendRequest(DocumentHighlightRequest.type, {
          textDocument: { uri: l.uri },
          position: pos,
        });
        return Array.isArray(response) ? response as DocumentHighlight[] : [];
      } catch {
        return [];
      }
    }));
  ...
  async getImplementation(fp: string, line: number, col: number): Promise<Location[]> {
    const l = await this.loadFile(fp);
    if (!l) return [];
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    const supportedClients = this.getSupportedClients(l.clients, "goToImplementation");
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try { return this.normalizeLocs(await c.connection.sendRequest("textDocument/implementation", { textDocument: { uri: l.uri }, position: pos })); }
      catch { return []; }
    }));
    return results.flat();
  }
  ...
  async getWorkspaceSymbols(fp: string, query = ""): Promise<SymbolInformation[]> {
    const absPath = this.resolve(fp);
    const clients = await this.getClientsForFile(absPath);
    if (!clients.length) return getOrCreateAstService().getWorkspaceSymbols(absPath, query);

    const supportedClients = this.getSupportedClients(clients, "workspaceSymbol");
    if (!supportedClients.length) return getOrCreateAstService().getWorkspaceSymbols(absPath, query);
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
        const response = await c.connection.sendRequest("workspace/symbol", { query });
        return Array.isArray(response) ? response as SymbolInformation[] : [];
      } catch {
        return [];
      }
    }));
  ...
  async getFoldingRanges(fp: string): Promise<FoldingRange[]> {
    const l = await this.loadFile(fp);
    if (!l) return getOrCreateAstService().getFoldingRanges(this.resolve(fp));
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const supportedClients = this.getSupportedClients(l.clients, "foldingRange");
    if (!supportedClients.length) return getOrCreateAstService().getFoldingRanges(l.absPath);
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
        const response = await c.connection.sendRequest(FoldingRangeRequest.type, {
          textDocument: { uri: l.uri },
        });
        return Array.isArray(response) ? response as FoldingRange[] : [];
      } catch {
        return [];
      }
    }));
  ...
  async prepareCallHierarchy(fp: string, line: number, col: number): Promise<CallHierarchyItem[]> {
    const l = await this.loadFile(fp);
    if (!l) return [];
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    const supportedClients = this.getSupportedClients(l.clients, "prepareCallHierarchy");
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
        const response = await c.connection.sendRequest("textDocument/prepareCallHierarchy", {
          textDocument: { uri: l.uri },
          position: pos,
        });
        return Array.isArray(response) ? response as CallHierarchyItem[] : [];
      } catch {
        return [];
      }
    }));
  ...
  async getIncomingCalls(item: CallHierarchyItem): Promise<CallHierarchyIncomingCall[]> {
    const absPath = this.resolve(uriToPath(item.uri));
    const clients = await this.getClientsForFile(absPath);
    if (!clients.length) return [];

    const supportedClients = this.getSupportedClients(clients, "incomingCalls");
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
        const response = await c.connection.sendRequest("callHierarchy/incomingCalls", { item });
        return Array.isArray(response) ? response as CallHierarchyIncomingCall[] : [];
      } catch {
        return [];
      }
    }));
  ...
  async getOutgoingCalls(item: CallHierarchyItem): Promise<CallHierarchyOutgoingCall[]> {
    ...
    const supportedClients = this.getSupportedClients(clients, "outgoingCalls");
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
        const response = await c.connection.sendRequest("callHierarchy/outgoingCalls", { item });
        return Array.isArray(response) ? response as CallHierarchyOutgoingCall[] : [];
      } catch {
        return [];
      }
    }));
  ...
  async rename(fp: string, line: number, col: number, newName: string): Promise<WorkspaceEdit | null> {
    ...
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    for (const c of this.getSupportedClients(l.clients, "rename")) {
      if (c.closed) continue;
      try {
  ...
  async getCodeActions(fp: string, startLine: number, startCol: number, endLine?: number, endCol?: number): Promise<(CodeAction | Command)[]> {
    ...
    for (const c of this.getSupportedClients(l.clients, "codeAction")) {
    ...
    const supportedClients = this.getSupportedClients(l.clients, "codeAction");
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try {
  ...
  async getTypeDefinition(fp: string, line: number, col: number): Promise<Location[]> {
    const l = await this.loadFile(fp);
    if (!l) return [];
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    const supportedClients = this.getSupportedClients(l.clients, "typeDefinition");
    const results = await Promise.all(supportedClients.map(async c => {
      if (c.closed) return [];
      try { return this.normalizeLocs(await c.connection.sendRequest("textDocument/typeDefinition", { textDocument: { uri: l.uri }, position: pos })); }
      catch { return []; }
    }));
    return results.flat();
  }

  async prepareRename(fp: string, line: number, col: number): Promise<{ placeholder?: string; range: { start: { line: number; character: number }; end: { line: number; character: number } } } | null> {
    const l = await this.loadFile(fp);
    if (!l) return null;
    await this.openOrUpdate(l.clients, l.absPath, l.uri, l.langId, l.content);
    const pos = this.toPos(line, col);
    for (const c of this.getSupportedClients(l.clients, "prepareRename")) {
      if (c.closed) continue;
      try {
        const result = await c.connection.sendRequest("textDocument/prepareRename", {
          textDocument: { uri: l.uri },
          position: pos,
        }) as { placeholder?: string; range?: { start: { line: number; character: number }; end: { line: number; character: number } } } | { start: { line: number; character: number }; end: { line: number; character: number } } | null;
        if (!result) continue;
        if ("range" in result && result.range) return { range: result.range, placeholder: result.placeholder };
        if ("start" in result && "end" in result) return { range: result };
      } catch {}
    }
    return null;
  }
