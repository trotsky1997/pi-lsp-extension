...
const analyzerHookModeSchema = z.enum([
	"write",
	"edit_write",
	"agent_end",
	"disabled",
...
const resolvedSettingsOverrides = new Map<string, ResolvedLSPSettings>();

function normalizeOverrideKey(cwd: string): string {
	return path.resolve(cwd);
}

export function setResolvedLspSettingsOverride(
	cwd: string,
	settings: ResolvedLSPSettings | undefined,
): void {
	const key = normalizeOverrideKey(cwd);
	if (!settings) {
		resolvedSettingsOverrides.delete(key);
		return;
	}
	resolvedSettingsOverrides.set(key, settings);
}

export function getResolvedLspSettingsOverride(
	cwd: string,
): ResolvedLSPSettings | undefined {
	return resolvedSettingsOverrides.get(normalizeOverrideKey(cwd));
}

export function clearResolvedLspSettingsOverrides(): void {
	resolvedSettingsOverrides.clear();
}
...
export function getFormatterEnv(
...
export function loadResolvedLspSettings(
	...
	const override = getResolvedLspSettingsOverride(cwd);
	...
	const globalServers = isPlainObject(
		(globalSettings as { lsp?: { servers?: unknown } }).lsp?.servers,
	)
		? ((
				globalSettings as {
					lsp?: { servers?: Record<string, LSPServerSettings> };
				}
			).lsp?.servers ?? {})
		: {};
	const projectServers = isPlainObject(
		(projectSettings as { lsp?: { servers?: unknown } }).lsp?.servers,
	)
		? ((
				projectSettings as {
					lsp?: { servers?: Record<string, LSPServerSettings> };
		...
		: {};
	const globalFormatters = isPlainObject(
		(globalSettings as { formatter?: { formatters?: unknown } }).formatter
			?.formatters,
	)
		? ((
				globalSettings as {
					formatter?: { formatters?: Record<string, FormatterSettings> };
				}
			).formatter?.formatters ?? {})
		: {};
	const projectFormatters = isPlainObject(
		(projectSettings as { formatter?: { formatters?: unknown } }).formatter
			?.formatters,
	)
		? ((
				projectSettings as {
		...
		: {};
	const globalAnalyzersSection = (
		globalSettings as { analyzer?: { analyzers?: unknown; tools?: unknown } }
	).analyzer;
	const projectAnalyzersSection = (
		projectSettings as { analyzer?: { analyzers?: unknown; tools?: unknown } }
	).analyzer;
	...
	const globalHookMode = (globalSettings as { lsp?: { hookMode?: HookMode } })
		.lsp?.hookMode;
	const projectHookMode = (projectSettings as { lsp?: { hookMode?: HookMode } })
		.lsp?.hookMode;
	const globalEnabled = (globalSettings as { lsp?: { enabled?: boolean } }).lsp
		?.enabled;
	const projectEnabled = (projectSettings as { lsp?: { enabled?: boolean } })
		.lsp?.enabled;
	const globalPythonProvider = (
		globalSettings as { lsp?: { python?: { provider?: PythonProvider } } }
	).lsp?.python?.provider;
	const projectPythonProvider = (
		projectSettings as { lsp?: { python?: { provider?: PythonProvider } } }
	).lsp?.python?.provider;
	const globalFormatterEnabled = (
		globalSettings as { formatter?: { enabled?: boolean } }
	).formatter?.enabled;
	const projectFormatterEnabled = (
		projectSettings as { formatter?: { enabled?: boolean } }
	).formatter?.enabled;
	const globalFormatterHookMode = (
		globalSettings as { formatter?: { hookMode?: FormatterHookMode } }
	).formatter?.hookMode;
	const projectFormatterHookMode = (
		projectSettings as { formatter?: { hookMode?: FormatterHookMode } }
	).formatter?.hookMode;
	const globalAnalyzerEnabled = (
		globalSettings as { analyzer?: { enabled?: boolean } }
	).analyzer?.enabled;
	const projectAnalyzerEnabled = (
		projectSettings as { analyzer?: { enabled?: boolean } }
	).analyzer?.enabled;
	const globalAnalyzerHookMode = (
		globalSettings as { analyzer?: { hookMode?: AnalyzerHookMode } }
	).analyzer?.hookMode;
	const projectAnalyzerHookMode = (
		projectSettings as { analyzer?: { hookMode?: AnalyzerHookMode } }
	).analyzer?.hookMode;
	...
	return {
		...
		formatterHookMode:
			projectFormatterHookMode ?? globalFormatterHookMode ?? "write",
		formatters,
		analyzerEnabled: projectAnalyzerEnabled ?? globalAnalyzerEnabled ?? true,
		analyzerHookMode:
			projectAnalyzerHookMode ?? globalAnalyzerHookMode ?? "agent_end",
		analyzers,
	};
}
