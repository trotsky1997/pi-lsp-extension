...
export class DapClient {
	...
	static async connect({
		...
		const { readable, writeSink, socket } = await connectTcp(host, port, () => exited.resolve());
		const proc = {
	...
	static async #spawnTcp({ adapter, cwd, socketReadyTimeoutMs }: DapSpawnOptions): Promise<DapClient> {
		...
		// adapter gets the same port via ${port} substitution.
		// ponytail: Node doesn't expose Bun.listen's port-only reservation;
		// we open a net server, read the port, close it, and pass to adapter.
		// There's a small TOCTOU window where another process could grab the
		// port — same as omp's original. Acceptable for local single-user use.
		const tmpServer = net.createServer(() => {});
		tmpServer.listen(0, host);
		const port = (tmpServer.address() as net.AddressInfo).port;
		tmpServer.close();
	...
	static async #spawnSocketUnix({
		...
		try {
			...
			const { readable, writeSink, socket } = await connectUnix(socketPath, undefined);
	...
	static async #spawnSocketClientAddr({
		...
		const { promise: connPromise, resolve: resolveConn } = Promise.withResolvers<SocketTransport>();

		// Listen on port 0 (OS picks a free port), accept one connection.
		const listener = listenAndAccept(transport => resolveConn(transport));
		...
		try {
			const { readable, writeSink, socket } = await Promise.race([connPromise, timeoutPromise]);
			const client = new DapClient(adapter, cwd, proc, { readable, writeSink, socket });
			proc.exited.then(() => client.#handleProcessExit());
			void client.#startMessageReader();
			return client;
...
// ── socket helpers (Node net-based, replacing Bun.connect/Bun.listen) ───────
// These use the abstractions from _deps.ts (connectTcp, connectUnix,
// listenAndAccept, isUnixSocketReady, sleep) so the engine has zero direct
 ...
 * first connect. Best-effort: resolves on the banner, on process exit, or on
 * timeout. Also drains stdout for the wait's duration: in tcp mode the DAP
 * protocol flows over the socket, so nothing else consumes the adapter stdout.
 */
export async function waitForTcpServerListening(
	...
	await Promise.race([ready.promise, sleep(timeoutMs)]);
}
