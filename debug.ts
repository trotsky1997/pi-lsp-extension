...
const debugSchema = Type.Object({
	action: StringEnum(ACTIONS, { description: "debug action to perform" }),
	program: Type.Optional(Type.String({ description: "debug target path; Delve accepts Go package directories" })),
	args: Type.Optional(Type.Array(Type.String(), { description: "program arguments" })),
	adapter: Type.Optional(Type.String({ description: "configured adapter id (gdb, lldb-dap, debugpy, dlv, rdbg, or dap.json entry)" })),
	...
	arguments: Type.Optional(Type.Record(Type.String(), Type.Unknown(), { description: "custom request arguments" })),
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Debug",
		description:
		"Drive a real debugger (lldb-dap, gdb, dlv, debugpy, codelldb, rdbg, …) via DAP. " +
		"Launch or attach to a program, set breakpoints, step, inspect stack/variables/memory, evaluate expressions. " +
		"Sessions persist across calls. Actions: launch, attach, set_breakpoint, remove_breakpoint, continue, step_*, " +
		"pause, evaluate, stack_trace, threads, scopes, variables, disassemble, read_memory, write_memory, " +
		"modules, loaded_sources, custom_request, output, terminate, sessions, and data/instruction breakpoint variants.",
	promptSnippet:
		"Attach lldb/gdb/dlv/debugpy and step through a real program: launch, set breakpoints, inspect stack/variables/memory",
	promptGuidelines: [
		"Use the debug tool to drive a real debugger via DAP — not print statements. Launch or attach once, then step and inspect across calls.",
		"Actions are stateful: launch/attach creates a session; subsequent stack_trace/threads/scopes/variables/evaluate operate on it. Use sessions to list active ones.",
		"For a segfault or crash: attach with the adapter matching the language (lldb-dap for C/C++/Rust, debugpy for Python, dlv for Go), then stack_trace + variables to read the faulting frame.",
		"Set breakpoints before continue: set_breakpoint (file+line) then continue. Breakpoints persist across steps.",
		"Adapters must be installed and on PATH. If launch fails with 'not available', the adapter binary is missing — check getConfiguredAdapters via sessions.",
	],
