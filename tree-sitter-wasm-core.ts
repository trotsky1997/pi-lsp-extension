...
const TREE_SITTER_WASM_ROOT = path.dirname(require.resolve("@vscode/tree-sitter-wasm/package.json"));
...
const [javascriptLanguage, typescriptLanguage, tsxLanguage, pythonLanguage] = await Promise.all([
  Language.load(path.join(TREE_SITTER_WASM_DIR, "tree-sitter-javascript.wasm")),
  Language.load(path.join(TREE_SITTER_WASM_DIR, "tree-sitter-typescript.wasm")),
...
function visitTree(node: WasmNode, visitor: (node: WasmNode) => boolean | void): void {
...
function findNamedNodeAtPosition(root: WasmNode, line: number, character: number): WasmNode | null {
  ...
  while (node && !isIdentifierLike(node)) {
    const child = node.namedChildren.find((candidate) => candidate !== null && pointInNode(candidate, point) && isIdentifierLike(candidate));
...
export class TreeSitterManager {
  ...
  private loadQuery(config: LanguageConfig, kind: "tags" | "locals"): WasmQuery | null {
  ...
  private getContainingScopes(locals: LocalCapture[], node: WasmNode): WasmNode[] {
    return locals
  ...
  private findLocalDefinitionScope(locals: LocalCapture[], target: WasmNode): WasmNode | null {
  ...
  private collectLocalMatches(locals: LocalCapture[], scope: WasmNode, text: string): LocalCapture[] {
  ...
  getDocumentHighlights(filePath: string, line: number, character: number): DocumentHighlight[] {
    ...
    const fallbackScope = closestNodeOfTypes(target, [
