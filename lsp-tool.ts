...
const LspParams = Type.Object({
	...
	filePath: Type.Optional(
		Type.String({ description: "Absolute or relative path to the file." }),
	),
	filePaths: Type.Optional(
		Type.Array(Type.String(), {
			description: "File paths for workspaceDiagnostics.",
		}),
	),
	line: Type.Optional(Type.Number({ description: "1-based line number." })),
	character: Type.Optional(
		Type.Number({ description: "1-based character offset." }),
	),
	endLine: Type.Optional(
		Type.Number({ description: "1-based end line for range operations." }),
	),
	endCharacter: Type.Optional(
		Type.Number({ description: "1-based end character for range operations." }),
	),
	newName: Type.Optional(
		Type.String({ description: "New symbol name for rename." }),
	),
	severity: Type.Optional(
		StringEnum(SEVERITY_FILTERS, {
			description:
				'Filter diagnostics: "all" | "error" | "warning" | "info" | "hint".',
		}),
	),
});
...
> = {
	...
	documentHighlight: {
		singular: "document highlight",
		plural: "document highlights",
	},
...
function countUniqueFilesFromLocations(
...
function countWorkspaceEditChanges(edit: WorkspaceEdit | null | undefined): {
	fileCount: number;
	resultCount: number;
} {
	...
	if (edit.changes) {
		for (const edits of Object.values(edit.changes) as Array<
			Array<{
				newText: string;
...
export function formatCodeActions(
	actions: Array<CodeAction | Command>,
): string {
...
export function buildOperationSummary(
	...
	if (details.backend === "tree-sitter")
		summary = `Tree-sitter fallback: ${summary}`;
	return summary;
}

export function withBackendLabel(
	text: string,
	backend: OperationBackend | undefined,
): string {
...
function buildResult(
	text: string,
	details: LspResultDetails | Record<string, unknown>,
) {
	return {
...
async function ensureOpenFile(
	...
	try {
		...
		if (!opened) {
			throw new Error(
				`No LSP server available for file type: ${path.extname(absolutePath) || "<unknown>"}`,
			);
		}
	} finally {
		await handle.close();
...
function formatOperationResult(
	...
	switch (operation) {
		case "goToDefinition": {
			...
			return {
				...
				details: {
					...
					fileCount: countUniqueFilesFromLocations(rawResults),
				},
			};
		}
		case "findReferences": {
			...
			return {
				text: withBackendLabel(
					formatFindReferencesResult(locations, cwd),
					backend,
				),
				details: {
					operation,
					...
					fileCount: countUniqueFilesFromLocations(locations),
				},
			};
		}
		case "hover": {
		...
		case "documentHighlight": {
			const highlights =
				(result as DocumentHighlight[] | null | undefined) ?? [];
			return {
				text: withBackendLabel(
					formatDocumentHighlightResult(highlights),
					backend,
				),
				details: {
					operation,
					...
					fileCount: highlights.length > 0 ? 1 : 0,
				},
			};
		}
		case "documentSymbol": {
			const symbols =
				(result as
					| Array<DocumentSymbol | SymbolInformation>
					| null
					| undefined) ?? [];
		...
		case "workspaceSymbol": {
			...
			return {
				text: withBackendLabel(
					formatWorkspaceSymbolResult(symbols, cwd),
					backend,
				),
				details: {
					operation,
		...
		case "goToImplementation": {
			const locations =
				(result as Array<Location | LocationLink> | null | undefined) ?? [];
			return {
				text: withBackendLabel(
					formatGoToDefinitionResult(
						locations as Location[] | LocationLink[],
						cwd,
					),
					backend,
				),
		...
		case "prepareCallHierarchy": {
			...
			return {
				text: withBackendLabel(
					formatPrepareCallHierarchyResult(items, cwd),
					backend,
				),
				details: {
					operation,
					...
					fileCount: countUniqueFilesFromCallItems(items),
				},
			};
		}
		case "incomingCalls": {
			const calls =
				(result as CallHierarchyIncomingCall[] | null | undefined) ?? [];
			return {
				...
				details: {
					...
					fileCount: countUniqueFilesFromIncomingCalls(calls),
				},
			};
		}
		case "outgoingCalls": {
			const calls =
				(result as CallHierarchyOutgoingCall[] | null | undefined) ?? [];
			return {
				...
				details: {
					...
					backend,
					resultCount: calls.length,
					fileCount: countUniqueFilesFromOutgoingCalls(calls),
				},
			};
		}
		case "signatureHelp": {
		...
		case "prepareRename": {
			...
			return {
				...
				details: {
					...
					filePath,
					backend,
					resultCount: renameTarget ? 1 : 0,
					fileCount: renameTarget ? 1 : 0,
				},
			};
		}
		case "foldingRange": {
			...
			return {
				...
				details: {
					...
					fileCount: ranges.length > 0 ? 1 : 0,
				},
			};
		}
		case "codeAction": {
			const actions =
				(result as Array<CodeAction | Command> | null | undefined) ?? [];
			return {
				...
				details: {
					...
					fileCount: actions.length > 0 ? 1 : 0,
				},
			};
		}
		default:
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(
			...
			const { ctx, signal } = normalizeExecuteArgs(
				onUpdateArg,
				ctxArg,
				signalArg,
			);
			...
			try {
				...
				switch (params.operation) {
					case "diagnostics": {
						const backend =
							(await abortable(
								manager.getOperationBackend(params.filePath!, params.operation),
								signal,
							)) ?? undefined;
						const result = await abortable(
							manager.touchFileAndWait(
								params.filePath!,
								diagnosticsWaitMsForFile(params.filePath!),
							),
							signal,
						);
					...
					case "workspaceDiagnostics": {
						const waitMs = Math.max(
							...params.filePaths!.map(diagnosticsWaitMsForFile),
						);
						const result = await abortable(
							manager.getDiagnosticsForFiles(params.filePaths!, waitMs),
							signal,
						);
						formatted = {
							text: (() => {
								...
								for (const item of result.items) {
									const display = path.isAbsolute(item.file)
							...
							details: {
								operation: params.operation,
								resultCount: result.items.reduce(
									(total, item) =>
										total +
										filterDiagnosticsBySeverity(item.diagnostics, severity)
											.length,
									0,
								),
								fileCount: result.items.filter(
									(item) =>
										filterDiagnosticsBySeverity(item.diagnostics, severity)
											.length > 0,
								).length,
					...
					default: {
						...
						if (!backend) {
							return buildResult(
								`Operation ${params.operation} is not supported by the active LSP server for ${params.filePath}.`,
								{
									operation: params.operation,
									filePath: params.filePath,
									resultCount: 0,
						...
						await abortable(
							ensureOpenFile(manager, params.filePath!, ctx.cwd),
							signal,
						);
						...
						switch (params.operation) {
							...
							case "documentSymbol":
								result = await abortable(
									manager.getDocumentSymbols(params.filePath!),
									signal,
								);
								break;
							case "workspaceSymbol":
								result = await abortable(
									manager.getWorkspaceSymbols(params.filePath!),
									signal,
								);
								break;
							case "goToImplementation":
								result = await abortable(
									manager.getImplementation(
										params.filePath!,
										params.line!,
										params.character!,
									),
							...
							case "incomingCalls": {
								...
								result = items.length
									? await abortable(manager.getIncomingCalls(items[0]!), signal)
									: [];
								break;
							}
							case "outgoingCalls": {
								...
								result = items.length
									? await abortable(manager.getOutgoingCalls(items[0]!), signal)
									: [];
								break;
							}
							case "signatureHelp":
								result = await abortable(
									manager.getSignatureHelp(
										params.filePath!,
										params.line!,
										params.character!,
									),
							...
							case "foldingRange":
								result = await abortable(
									manager.getFoldingRanges(params.filePath!),
									signal,
								);
								break;
							case "codeAction":
								result = await abortable(
									manager.getCodeActions(
										params.filePath!,
										params.line!,
										params.character!,
										params.endLine,
						...
						formatted = formatOperationResult(
							params.operation,
							result,
							params.filePath,
							ctx.cwd,
		...
		renderCall(args: unknown, theme: any) {
			...
			let text =
				...
				theme.fg("accent", params.operation || "...");
